...
def rootrearr(__a,__x:list[Decimal]|tuple[Decimal,...],__p=0.01,__pr=getpr())->dict:
    try:
        ...
        for i in range(len(__a.getf())):
            l1=list();
            if (f:=funcutils.rearr(__a,i,pr1)) is None:continue;
            for j in x:
                if (a:=abs(f.df(j,pr1))) is None:continue;
                if a<=1:l1.append(j)
        ...
        for i in l:
            ...
            for j in i[1]:
                ...
                while p>__p:
                    f1=f.f(f1,pr1);c+=1;
                    if (p:=abs(__a.f(f1,pr1))) is None:break;
                if p is not None:l2.append({'rearrange pos':i[2],'iterations':c,'start value':j,'value':(deciml(f1,__pr),deciml(__a.f(f1,pr1),__pr))});
            if len(l2)>0:l1.append(tuple(l2))
        return tuple(l1)
        ...
        for i in ln:
            ...
            for j in lp:
                ...
                while p>__p:
                    ...
                    if (p:=abs(p)) is None:break;
                if p is not None:l1.append({'iterations':c,'start value':(i,j),'value':(deciml(x,__pr),deciml(__a.f(x,pr1),__pr))});
            if len(l1)>0:l.append(tuple(l1))
        return tuple(l)
...
def nriter(__a,__x:list[Decimal]|tuple[Decimal,...],__p=0.01,__pr=getpr())->dict:
    try:
        ...
        for i in x:
            ...
            while p>__p:
                x=alg.sub(a,alg.div(__a.f(a,pr1),__a.df(a,pr1),pr1),pr=pr1);c+=1;a=x;
                if (p:=abs(__a.f(x,pr1))) is None:break;
            if p is not None:l.append({'iterations':c,'start value':i,'value':(deciml(x,__pr),deciml(__a.f(x,pr1),__pr))});
        return tuple(l)
...
#     for i in p.items():
#         p[i[0]] = nriter(i[1], [-10, 0, 10], 0.001)
#     for i in p.items():
#         print(i,'\n\n')
# p = rootrearr(poly((1,5), (-2,2), (-3,0)), [1.7,], 0.00001)
# print(p)
# p = rootrearr(poly((4,0), (-4,1), (1,2),), [i - 5 for i in range(10)],0.001)
# print(p)
# p = bchop(poly((1,5), (-2,2), (-3,0)),[1.7,1,-1], 0.00001)
# print(p)
# p = lininter(poly((1,5), (-2,2), (-3,0)),[1.7,1,-1], 0.00001)
# print(p)
# p = nriter(poly((1,5), (-2,2), (-3,0)),[1.7,1,-1], 0.00001)
# print(p)